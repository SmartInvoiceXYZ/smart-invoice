...
export const getGraphUrl = (chainId: ChainId) => graphUrls[chainId] || graphUrls[DEFAULT_CHAIN_ID];

export const getExplorerUrl = (chainId: ChainId) => explorerUrls[chainId] || explorerUrls[DEFAULT_CHAIN_ID];

export const getRpcUrl = (chainId: ChainId) => rpcUrls[chainId] || rpcUrls[DEFAULT_CHAIN_ID];

export const getResolvers = (chainId: ChainId) => resolvers[chainId] || resolvers[DEFAULT_CHAIN_ID];

export const getResolverInfo = (chainId: ChainId, resolver: string) =>
  (resolverInfo[chainId] || resolverInfo[DEFAULT_CHAIN_ID])[resolver];

export const getTokens = (chainId: any, allTokens: any) =>
  allTokens[chainId] || allTokens[DEFAULT_CHAIN_ID];

export const getTokenInfo = (chainId: any, token: any, tokenData: any) => {
...
export const getWrappedNativeToken = (chainId: ChainId) => wrappedNativeToken[chainId] || wrappedNativeToken[DEFAULT_CHAIN_ID];

export const getNativeTokenSymbol = (chainId: ChainId) => nativeSymbols[chainId] || nativeSymbols[DEFAULT_CHAIN_ID];
...
export const isKnownResolver = (chainId: any, resolver: any) =>
  getResolvers(chainId).indexOf(resolver.toLowerCase()) !== -1;

export const getResolverString = (chainId: any, resolver: any) => {
...
export const formatTokens = (object: any) => {
  const tokenObject = {};
  for (const [key, value] of Object.entries(object)) {
    const tokenArray = [];
    // @ts-expect-error TS(2769): No overload matches this call.
...
export const getTokenSymbol = (token: any, chainId: any, tokenData: any) =>
