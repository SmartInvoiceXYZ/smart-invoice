...
import { SmartInvoiceUpdatableV201 } from '../../../types/templates/SmartInvoiceUpdatableV201/SmartInvoiceUpdatableV201';
import { handleIpfsDetails } from './ipfs';

function fetchEscrowInfo(address: Address): InvoiceObject {
  let updatableInstance = SmartInvoiceUpdatableV201.bind(address);

  let invoiceObject = new InvoiceObject();

  let client = updatableInstance.try_client();
  let clientReceiver = updatableInstance.try_clientReceiver();
  ...
  if (!clientReceiver.reverted) {
    invoiceObject.clientReceiver = clientReceiver.value;
  }
...
export function updateUpdatableV2Info(
  address: Address,
  invoice: Invoice,
): Invoice {
  ...
  invoice.clientReceiver = invoiceObject.clientReceiver;
