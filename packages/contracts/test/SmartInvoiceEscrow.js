...
describe("SmartInvoiceEscrow", function() {
  ...
  async function deployInvoice(
    ...
    await factory.addImplementation(
    ...
    const data = ethers.utils.AbiCoder.prototype.encode(
      ...
      [
        ...
        _resolver,
        _token,
        _terminationTime, // exact termination date in seconds since epoch
        _details,
        _wrappedNativeToken,
        _requireVerification,
      ],
  ...
  const getLockedEscrow = async (
    ...
    _resolutionRate,
    _details,
    _mockWrappedNativeToken,
    _value = 0,
  ) => {
    const currentTime = await currentTimestamp();
    let newInvoice = await _SmartInvoiceEscrow.deploy();
    await newInvoice.deployed();
    const initReceipt = await deployInvoice(
      newInvoice,
      _client.address,
      _provider.address,
      _resolverType,
      ...
      _resolutionRate,
      _details,
      _mockWrappedNativeToken.address,
      false,
    );
    const newInvoiceAddress = await awaitInvoiceAddress(
    ...
    await expect(receipt)
      ...
      .withArgs(_client.address, EMPTY_BYTES32);
    return newInvoice;
  };
  ...
  xit("Should revert init if resolutionRate is 0", async function() {
    const currentTime = await currentTimestamp();
    invoice = await SmartInvoiceEscrow.deploy();
    await invoice.deployed();
    const receipt = deployInvoice(
  ...
  it("Should revert resolve if awards do not add up", async function() {
    const newInvoice = await SmartInvoiceEscrow.deploy();
    await newInvoice.deployed();
    let lockedInvoice = await getLockedEscrow(
      SmartInvoiceEscrow,
      client,
      provider,
      individualResolverType,
