...
describe('SmartInvoiceSplitEscrow', function () {
  ...
  beforeEach(async function () {
    ...
    [client, provider, resolver, randomSigner, dao] = walletClients;
  ...
  it('Should deploy a SmartInvoice', async function () {
    ...
    expect(await invoice.read.dao()).to.equal(getAddress(dao.account.address));
  ...
  it('Should release', async function () {
    ...
    const beforeDaoBalance = await getBalanceOf(mockToken, dao.account.address);
    ...
    const afterDaoBalance = await getBalanceOf(mockToken, dao.account.address);
    expect(afterBalance).to.equal(beforeBalance + 9n);
  ...
  it('Should release full balance at last milestone', async function () {
    ...
    expect(await invoice.read.released()).to.equal(30);
    ...
    const afterDaoBalance = await getBalanceOf(mockToken, dao.account.address);
    expect(afterBalance).to.equal(beforeBalance + 27n);
    expect(afterDaoBalance).to.equal(beforeDaoBalance + 3n);
  });
  ...
  it('Should release full balance after all milestones are completed', async function () {
    ...
    await setBalanceOf(mockToken, invoice.address, 20);
    receipt = await invoice.write.release();
    expect(await invoice.read.released()).to.equal(50);
    expect(await invoice.read.milestone()).to.equal(2);
    ...
    expect(afterDaoBalance).to.equal(beforeDaoBalance + 5n);
  });
  ...
  it('Should resolve with correct rewards', async function () {
    ...
    const daoBeforeBalance = await getBalanceOf(mockToken, dao.account.address);
    ...
    const daoAfterBalance = await getBalanceOf(mockToken, dao.account.address);
    ...
    expect(daoAfterBalance).to.be.equal(daoBeforeBalance + 9n);
    expect(providerAfterBalance).to.be.equal(providerBeforeBalance + 81n);
