    ...
    IDisputeResolver
{
    ...
    uint256 internal constant _WINNER_STAKE_MULTIPLIER = 3000; // 30% of the appeal fee
    uint256 internal constant _LOSER_STAKE_MULTIPLIER = 7000; // 70% of the appeal fee
    uint256 internal constant _LOSER_APPEAL_PERIOD_MULTIPLIER = 5000; // losers: first 50% of the appeal window

    /// @notice Number of ruling options available to arbitrators: client vs provider; plus 0 = \"refused/split\"
    uint256 private constant _NUM_RULING_OPTIONS = 2;

    /// @notice Arbitrator resolver for _disputes
    ...
    DisputeData[] private _disputes;
    /// @notice Crowdfunded appeal rounds for each dispute
    mapping(uint256 => Round[]) private _appealRoundsMap;
    ...
    function lock(
        ...
        uint256 localDisputeId = _disputes.length;
        ...
        _appealRoundsMap[localDisputeId].push();
    ...
    function submitEvidence(
        ...
        if (_localDisputeId >= _disputes.length) revert IncorrectDisputeId();
        DisputeData storage disputeData = _disputes[_localDisputeId];
    ...
    function fundAppeal(
        ...
        if (_localDisputeId >= _disputes.length) revert IncorrectDisputeId();
        if (!locked) revert NotLocked();
        if (_ruling == 0 || _ruling > _NUM_RULING_OPTIONS)
        ...
        Round[] storage rounds = _appealRoundsMap[_localDisputeId];
    ...
    function withdrawFeesAndRewards(
        ...
        amount = _withdrawFeesAndRewards(
            _localDisputeId,
            _contributor,
            _roundIndex,
            _ruling
    ...
    function rule(
        ...
        if (_ruling > _NUM_RULING_OPTIONS) revert InvalidRuling(_ruling);

        uint256 localDisputeId = externalIDtoLocalID[_externalDisputeId];
        DisputeData storage disputeData = _disputes[localDisputeId];
        if (disputeData.isRuled) revert DisputeAlreadyRuled();
        ...
        Round[] storage rounds = _appealRoundsMap[localDisputeId];
    ...
    function getTotalWithdrawableAmount(
        ...
        if (_localDisputeId >= _disputes.length) return 0;
        if (locked) return 0;
        DisputeData storage disputeData = _disputes[_localDisputeId];
    ...
    {
        return (
            _WINNER_STAKE_MULTIPLIER,
            _LOSER_STAKE_MULTIPLIER,
            _LOSER_APPEAL_PERIOD_MULTIPLIER,
            _BPS_DENOMINATOR
        );
    }
    ...
    /// @notice Internal withdraw reimbursable fees and/or rewards from a specific round
    ...
    function _appealCost(
        ...
        return (
            appealFee,
            appealFee + ((appealFee * multiplier) / _BPS_DENOMINATOR)
        );
    }
