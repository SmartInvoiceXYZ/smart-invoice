    ...
    ReentrancyGuard
{
    ...
    IWRAPPED public immutable override WRAPPED_ETH;

    /// @notice Total count of invoices created by this factory
    uint256 public override invoiceCount = 0;
    ...
    mapping(bytes32 => uint256) public override currentVersions;
     ...
     * @return escrow The address of the created invoice
     */
    function createDeterministic(
        ...
        uint256 _version,
        bytes32 _salt
    ) external override nonReentrant returns (address escrow) {
    ...
    function createDeterministicAndDeposit(
        ...
        bytes32 _salt,
        uint256 _fundAmount
    ) external payable override nonReentrant returns (address escrow) {
        escrow = _createEscrow(
            _provider,
            _milestoneAmounts,
            _escrowData,
     ...
     * @return predicted The predicted address of the deterministic clone
     */
    function predictDeterministicAddress(
        ...
        bytes32 derivedSalt = _getDerivedSalt(
            _recipient,
            _amounts,
            _data,
            _escrowType,
     ...
     * @param _deployer The address that will deploy the contract (msg.sender for createDeterministic)
     * @return salt The derived salt that combines all parameters
     */
    function getDerivedSalt(
        ...
        bytes32 _salt,
        address _deployer
    ) external pure override returns (bytes32 salt) {
        salt = _getDerivedSalt(
            _recipient,
            _amounts,
            _data,
            _escrowType,
    ...
    /**
     * @notice Internal creates a new smart invoice instance deterministically using CREATE2
     * @param _recipient The address of the recipient (provider)
     * @param _amounts The array of amounts associated with the recipient
     * @param _data Additional data needed for initialization
     * @param _escrowType The type of the invoice (e.g., "ESCROW", "INSTANT")
    ...
    function _createEscrow(
        ...
        escrow = Clones.cloneDeterministic(_implementation, derivedSalt);

        _init(escrow, _recipient, _amounts, _data, _escrowType, _version);
    }
    ...
    /**
     * @notice Internal helper function to compute the derived salt used in deterministic deployments
     * @param _recipient The address of the recipient (provider)
     * @param _amounts The array of amounts associated with the recipient
     * @param _data Additional data needed for initialization
     * @param _escrowType The type of the invoice
