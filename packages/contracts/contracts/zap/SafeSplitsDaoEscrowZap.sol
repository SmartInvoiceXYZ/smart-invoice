...
import {
    ISafeSplitsDaoEscrowZap
} from "contracts/interfaces/ISafeSplitsDaoEscrowZap.sol";
...
contract SafeSplitsDaoEscrowZap is
    SafeSplitsEscrowZap,
    ISafeSplitsDaoEscrowZap
{
    struct DaoZapData {
    ...
    function setDao(address _dao) external override onlyRole(ADMIN) {
    ...
    function setSpoilsBPS(uint16 _spoilsBPS) external override onlyRole(ADMIN) {
        if (_spoilsBPS > _BPS_DENOMINATOR) revert InvalidSpoilsBPS();
    ...
    function _deploySplit(
        ...
        if (spoilsBPS == 0 || spoilsBPS >= _BPS_DENOMINATOR)
            revert InvalidSpoilsBPS();

        uint256 projectBPS = _BPS_DENOMINATOR - spoilsBPS;
        ...
        SplitV2Lib.Split memory daoSplitParams = SplitV2Lib.Split({
            ...
            totalAllocation: _BPS_DENOMINATOR,
            distributionIncentive: distributionIncentive
        });
